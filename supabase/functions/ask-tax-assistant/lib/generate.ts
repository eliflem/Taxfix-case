...
export async function generateAnswer(
  ...
  const knownGapsBlock = knownGaps.length
    ? `\nKnown limitations of your knowledge base — apply these behaviors if relevant, even though they may not be tier-obvious from the question alone:\n${knownGaps.map((g) => `- ${g.description.trim()} Behavior: ${g.behavior.trim()}`).join("\n")}\n`
    : "";
...
User's question: "${query}"`;
  ...
  return { text: res.text ?? "", citations: results.map((r) => r.chunk.sourceLabel) };
}

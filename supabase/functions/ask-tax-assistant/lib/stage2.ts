...
import { retrieveTopK, type RetrievalResult } from "./retrieval.ts";
...
export function checkGroundedness(answerText: string, retrievedContext: string): { grounded: boolean; ungroundedNumbers: string[] } {
  ...
  const ungrounded = answerNumbers.filter((n) => {
    ...
    return digits.length > 0 && !contextNormalized.includes(digits);
  });
...
export async function checkTemporalScope(query: string): Promise<{ flagged: boolean; note?: string }> {
  const config = await loadConfig();
